...
import {
  ...
  GetPlaylistMetaResponse,
  PostPlaylistResponse,
} from '@/api/type/response/playlist';
...
export const getPlaylistMeta = (playlistCode: string): Promise<GetPlaylistMetaResponse> => {
  return getFetch<GetPlaylistMetaResponse>(`/playlist/meta/${playlistCode}`).then(
    (res) => res ?? false,
  );
};

export const getPlaylistMetaPublic = (playlistCode: string): Promise<GetPlaylistMetaResponse> => {
  return getFetch<GetPlaylistMetaResponse>(`/playlist/meta/${playlistCode}/public`).then(
    (res) => res ?? false,
  );
};

export const getPlaylist = (playlistCode: string): Promise<GetPlaylistResponse> => {
  return getFetch<GetPlaylistResponse>(`/playlist/${playlistCode}`).then(
    (res) => transformPlaylist(res) ?? false,
  );
};

export const postPlaylistThumbnail = ({
  file,
}: PostPlaylistThumbnailRequest): Promise<{ result: string } | boolean> => {
  const formData = transformPlaylistThumbnail(file);

  return postFormFetch<{ result: string }>('/playlist/thumbnail', formData).then(
    (res) => res ?? false,
  );
};

export const getPlaylistHealth = (playlistCode: string): Promise<GetPlaylistHealthResponse> => {
  return getFetch<GetPlaylistHealthResponse>(`/playlist/${playlistCode}/health`).then(
    (res) => res ?? false,
  );
};

export const postPlaylist = (
  body: PostPlaylistRequest,
): Promise<PostPlaylistResponse | boolean> => {
  return postFetch<PostPlaylistResponse, string>('/playlist', body).then((res) => res ?? false);
};

export const postPlaylistNowPlaying = (
  playlistCode: string,
  videoCode: string,
